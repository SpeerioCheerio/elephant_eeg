...
thresholds_csv = base_path / 'metadata' / 'crl_sleep_thresholds.csv'
...
def preprocess(args):
    ...
    fig.savefig(f'/tmp/{fname}_raw.png')
    # Create epochs of 4s
    epoch_duration = 4.
    ...
    for i in range(len(epochs)):
        ...
        eeg_threshold = 1000e-6  #
        emg_threshold = 2200e-6  #
    ...
    os.makedirs(epochs_filename.parent, exist_ok=True)

    # compute the time shift in seconds
    first_samp_shift_sec = raw.first_samp / raw.info['sfreq']

    # shift the annotations
    # calculate the first_samp in seconds
    first_samp_shift_sec = raw.first_samp / raw.info['sfreq']

    # Shift annotations to match the data
    raw.annotations.onset += first_samp_shift_sec

    # Save the raw data, now with shifted annotations
    raw.save(raw_filename, overwrite=True)

    # Use .parent to get directory
    raw.save(raw_filename, overwrite=True)

    #raw_2 = mne.io.read_raw_fif(raw_filename, preload=True)


    epochs_with_events.save(epochs_filename, overwrite=True)
    print("#####", raw.info['description'])
    # epochs_with_events.plot_psd(fmin=1, fmax=50)
    # fig = epochs_with_events.plot_psd(fmin=1, fmax=50)
    # fig.savefig(f'/tmp/{fname}_epochs.png')


def main(num_processes=4):
    data_folder = data_path / 'crl_data' / 'mne_raw'
    save_folder = data_path / 'crl_data' / 'preprocessed'
