...
def extract_features(mne_file, epochs_duration: int, bands: List[Tuple[str, float, float]],
    ...
    samples_per_second = window * mne_file.info["sfreq"] # replace with actual sampling rate if different

    # Crop the file
    mne_file = mne_file.crop(tmin=0, tmax=samples_per_second / mne_file.info['sfreq'])
