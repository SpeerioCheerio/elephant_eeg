...
def generate_record_data(filename):
    """
    ...
    epoch_duration = 4.
    # load
    eeg_data, emg_data, info = load_record(filename)
    sfreq = info['sfreq']
    ...
    filtered_eeg = filter_data(eeg_data.astype(np.float64), sfreq=sfreq,
                               l_freq=5, h_freq=10,
                               method='fir', fir_design='firwin',
                               phase='zero', fir_window='hamming')
            ...
            for rat_id in rats:
                ...
                if filename.exists():
                    ...
                    if overwrite or not figure_save_path.exists():
                        # process and plot
                        print(filename)
                        processed_data = generate_record_data(filename)
